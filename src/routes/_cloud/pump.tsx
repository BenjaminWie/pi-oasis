...
function ManualControl({
  ...
  useEffect(() => {
    if (latestManual) {
      ...
      if (
        (mp?.action === "off" || mp?.action === "on") &&
        (latestManual.status === "done" || latestManual.status === "delivered")
      ) {
        if (mp?.action === "off") setLocalAction(null);
      }
...
function PumpPage() {
  ...
  return (
    <div className="px-5 space-y-5">
      ...
      {/* Manual control */}
      <ManualControl activeId={activeId} strategy={strategy} details={details} />
