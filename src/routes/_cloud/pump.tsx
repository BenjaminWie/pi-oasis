...
// Optimization: Move static fields outside the component to prevent re-creation on every render.
...
// Optimization: This sub-component isolates the 1-second 'now' timer.
// By moving the timer here, we prevent the entire PumpPage (including the expensive Recharts Chart)
// from re-rendering every second. This significantly reduces CPU load on low-power devices like Raspberry Pi.
function PumpManualAndDiagnostics({
  details,
  manualMut,
  saveMut,
  strategy,
  ...
  const activeMinutes = useMemo(() => {
    ...
    if (
      ...
      const completedAt =
        latestManual?.completed_at || (latestManual as any)?.delivered_at || latestManual?.created_at;
        ...
        <button
          onClick={() =>
            manualMut.mutate(
              ...
              {
                onSuccess: (data: any) => {
                  setLocalAction({ id: data.id, action: "off" });
                },
              },
            )
          }
...
function PumpPage() {
  ...
  const manualMut = useMutation({
    ...
    onSuccess: (data, vars) => {
      toast.success(
        vars.action === "on"
          ? `Befehl gesendet${vars.minutes ? ` (${vars.minutes} min)` : ""} — wartet auf Node-RED`
  ...
  // Optimization: Memoize derived event data to prevent expensive filtering/searching
  // on every render (especially important since we have a 1s timer on this page).
  ...
  const curTemp = useMemo(() => {
    const ev = pumpEvents.find((e) => (e.metrics as any)?.outside_temp !== undefined);
    return (ev?.metrics as any)?.outside_temp;
  }, [pumpEvents]);

  const curRain = useMemo(() => {
    const ev = pumpEvents.find((e) => (e.metrics as any)?.precipitation_mm !== undefined);
    return (ev?.metrics as any)?.precipitation_mm;
  }, [pumpEvents]);

  const curPv = useMemo(() => {
    const ev = pumpEvents.find((e) => (e.metrics as any)?.pv_surplus_watt !== undefined);
    return (ev?.metrics as any)?.pv_surplus_watt;
  }, [pumpEvents]);
