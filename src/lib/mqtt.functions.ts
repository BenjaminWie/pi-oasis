...
export const listMqttBrokers = createServerFn({ method: "GET" }).handler(async () => {
  return mockContainers
    .filter((c) => c.isMqtt || /mosquitto|emqx|hivemq|nanomq|vernemq/i.test(c.image) || c.ports.some((p) => p === "1883" || p === "8883"))
    .map((c) => ({ id: c.id, name: c.name, image: c.image, status: c.status, port: c.ports[0] ?? "1883" }));
});
...
export const pollMqttMessages = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{ messages: MqttMessage[] }> => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (data.topicFilter && data.topicFilter !== "#" && !m.topic.includes(data.topicFilter.replace(/[#+]/g, ""))) continue;
      messages.push(m);
    }
    return { messages };
  });
...
export const publishMqttMessage = createServerFn({ method: "POST" })
  .inputValidator((d: { brokerId: string; topic: string; payload: string; qos?: 0 | 1 | 2; retained?: boolean }) => d)
  .handler(async ({ data }) => {
    // On the Pi: client.publish(data.topic, data.payload, { qos, retain }).
